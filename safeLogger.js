...
function maskValue(value) {
  ...
  return `${value.slice(0, 4)}...${value.slice(-2)}`; // partial mask for debugging
}

function redactString(input) {
  let out = String(input ?? "");

  // redact known env var values if present in process.env
  ...
  // redact common Shopify token pattern
  out = out.replace(/shpat_[a-zA-Z0-9_-]+/g, "[REDACTED_SHOPIFY_TOKEN]");

  // redact Authorization/Bearer-like tokens in text blobs
  out = out.replace(/(Bearer\s+)[A-Za-z0-9\-._~+/]+=*/gi, "$1[REDACTED]");
  out = out.replace(/("X-Shopify-Access-Token"\s*:\s*")[^"]+("))/gi, '$1[REDACTED]$2');
...
function info(message, meta) {
  if (meta === undefined) {
    ...
    console.log(redactString(message), safeSerialize(meta));
  }
}
...
function warn(message, meta) {
  if (meta === undefined) {
    ...
    console.warn(redactString(message), safeSerialize(meta));
  }
}
...
function error(message, errOrMeta) {
  if (errOrMeta instanceof Error) {
    ...
    if (errOrMeta.stack) {
      console.error(redactString(errOrMeta.stack));
    }
    return;
  }
  ...
  if (errOrMeta === undefined) {
    ...
    console.error(redactString(message), safeSerialize(errOrMeta));
  }
}
